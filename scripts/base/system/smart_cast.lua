...
local order_id = require 'war3.basic.order_id'
...
local function get_smart_cast_type(name)
	local hero = yo.player._self.hero
	if not hero then
		return 0
	end
...
local function find_target(skill, x, y)
	--筛选出单位
	local target = yo.point.new(x, y)
	local g = yo.selector.new()
		: in_range(target, 200)
		: add_filter(game.target_filter(skill.owner, skill.target_data, true))
...
local function cast_spell(msg, hero, name, force)
	...
	if skl.target_type == yo.ability.TARGET_TYPE_POINT then
		...
		return true
	elseif skl.target_type == yo.ability.TARGET_TYPE_NONE then
		clean_last_skill()
		...
		if target or skl.target_type == yo.ability.TARGET_TYPE_UNIT_OR_POINT then
			local target = target or yo.point.new(x, y)
			if not on_can_order(skl, target) then
				return true
			end
			clean_last_skill()
...
local function is_select_hero()
	...
	--是不是自己的英雄
	if hero and hero == player._self.hero then
		return hero
	end
	return false
end
...
local function is_select_shop()
	...
	--是不是自己的英雄
	if shop and shop == player._self.shop then
		return true
	end
	return false
end
...
local function is_select_player_unit()
	...
	--是不是自己的英雄
	if u and u:get_owner() == player._self then
		return true
	end
	return false
end
...
local function is_select_off_line_hero()
	...
	if u == player._self.hero then
		return false
	end
	local p = u:get_owner()
	return jass.GetPlayerAlliance(p.handle, player._self.handle, 6) and u
end
...
local function select_hero()
	...
	--否则将选择切回自己的英雄
	local hero = player._self.hero
	if hero then
		--jass.SelectUnit(hero.handle, true)
		player._self:selectUnit(hero)
...
function message.hook(msg)
	...
	if msg.type == 'key_down' then
		...
		--tab
		if code == 515 then
			if game.multiboard then
				game.multiboard:minimize(false)
			end
			return false
		end
	end
	...
	if msg.type == 'mouse_ability' then
		...
		local name = game.id2string(ability)
		-- 鼠标左键按下
		if code == 1 then
			if ability == 0 then
				return true
