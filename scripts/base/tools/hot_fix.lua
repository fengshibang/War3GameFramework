...
local ver_name = game.VERSION
local dir_hot_fix = '东方战姬\\热补丁\\' .. ver_name .. '\\'

local logs = {}

local function save_logs(log) table.insert(logs, log) end

local function start_logs(p)
    local i = 0
    yo.timer.realTimer:loop(2000, function(t)
        i = i + 1
        local text = logs[i]
...
local function show_logs()
    yo.event:notify('玩家-注册英雄',
                    (function(trg, player) start_logs(player) end))
    for i = 1, 10 do
        local p = player[i]
        if p.hero then start_logs(p) end
    end
end
...
function hot_fix.main(god_p)
    ...
    for i = 1, 10 do
        ...
        if p:is_player() then
            p:sync({ver = hot_fix.my_ver}, function(data)
                count = count + 1
                if data then
                ...
                if count >= player.count_alive() then
                    load_hot_fix()
                end
            end)
        else
    ...
    function load_hot_fix()
        ...
        log.info(('[%s]同步热补丁,版本号为[%s]'):format(
                     hot_fix.player:getBaseName(), hot_fix.ver))

        -- 版本号最大的玩家同步热补丁
        if not hot_fix.player:is_player() then
            log.info(('[%s]离开游戏,同步失败'):format(
                         hot_fix.player:getBaseName()))
            return
        end

        -- 先同步签名文件
        hot_fix.player:syncText({sign = hot_fix.my_sign}, function(data)
            if not data then
                log.warn('签名文件同步失败')
                return
            end
            ...
            -- 再同步热补丁
            hot_fix.player:syncText({content = hot_fix.my_content},
                                    function(data)
                if not data then
                    log.warn('热补丁文件同步失败')
                    return
...
timer.realTimer:wait(0, function() hot_fix.main()
end)
