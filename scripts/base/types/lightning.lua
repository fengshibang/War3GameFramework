...
function mt:check_visible()
	local visible = self.keep_visible or (yo.player.self:is_visible(self.start) and yo.player.self:is_visible(self.target))
	if self.is_visible then
		if not visible then
...
function mt:move(start, target, oz1, oz2)
	...
	if not self.present or self:getAlpha() <= 0 then
		self.present = self.target:get_position()
	end
	if self.movetimes > 1 then
		local d = self.present * self.target:get_position()
		self.movespeed = d / (self.movetimes * 1.0) 
		
		self.present = self.present - { self.present / self.target:get_position(), self.movespeed }
	...
	local x1, y1, z1 = self.start:get_position():get(self.offset_z)
	local x2, y2, z2 = self.present:get_position():get(self.offset_z)
