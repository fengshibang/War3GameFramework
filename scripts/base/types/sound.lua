...
function sound:__call(data)
	if self.player and self.player ~= player._self then
		return
	end
	-- 音量[0-1]
	if data.volume then
...
--	路径
--	[占用规则]
function player._mt:play_sound(name, on_cover)
	...
	local dur = sound.get_duration(name)
	data.timer = timer.realTimer:wait(dur, function()
...
--以点为目标播放音效
function point._mt:play_sound(name)
	local snd = sound.pool_get(name) or sound.create(name)
	jass.SetSoundPosition(snd, self:get_position():get())
	jass.StartSound(snd)
	return setmetatable({handle = snd}, sound)
end

--以单位为目标播放音效
function unit._mt:play_sound(name)
	local snd = sound.pool_get(name) or sound.create(name)
...
--	[武器类型]
function unit._mt:get_weapon_sound(target, weapon_type, n)
