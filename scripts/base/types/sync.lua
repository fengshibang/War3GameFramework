...
sync.len = #sync.str
sync.strs = {}
for i = 1, sync.len do sync.strs[i] = sync.str:sub(i, i) end
...
--	同步完成后回调的函数
function player._mt:sync(data, func)
    -- print(('Start Sync: %s\t%s'):format(self:get(), func))
    if self:isObserver() or not self:is_player() then
        print(('sync.lua warning:player %d is not an alive player'):format(
                  self:get()))
        return
    end
    local i = 0
    ...
    local times = 0
    -- 开启计时器,等待同步完成
    timer.realTimer:loop(100, function(t)
        -- 检查是否同步完成
...
--	同步完成时的回调函数
function player._mt:syncText(data, func)
    ...
    for i = 1, math.ceil(#all_text / 4) do
        ...
        local int = war3.string2id(text) - 2 ^ 31
        table.insert(ints, int)
    end

    -- 先同步长度
    self:sync({count = #ints}, function(data)
        if not data then
            func(false)
            return
        end
        if not self:is_self() then
            for i = 1, data.count do ints[i] = 0 end
        end
        -- 同步所有的整数
        self:sync(ints, function(data)
            if not data then
                func(false)
                return
            end
            ...
            for i = text_count * 2 + 2, #data do
                table.insert(texts, war3.id2string(data[i] + 2 ^ 31))
            end
