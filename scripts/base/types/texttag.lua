...
function mt:set_position(position)
	jass.SetTextTagPos(self.handle, (position or self.position):get_position():get())
end
...
function mt:setShow(show)
	...
	local function is_visible()
		if self.target then 
			return self.target:is_visible(player._self)
		else 
			return player._self:is_visible(self.position)
		end
	end
	if has_flag(show, texttag.SHOW_FOG) or is_visible() then
		if has_flag(show, texttag.SHOW_ALL) then
			...
			if has_flag(show, texttag.SHOW_SELF) and player._self == self.player then
				flag = true
			else
				if has_flag(show, texttag.SHOW_ALLY) and not self.player:is_enemy(player._self) then
					flag = true
				end
			end
		end
